...
function generateIndex() {
  ...
  const files = readdirSync(docsDir)
    .filter((f) => f.startsWith("tcm-") && f.endsWith(".html") && f !== "index.html")
    .sort()
    .reverse();

  const links = files.slice(0, 60)
    .map((f) => {
      const dateStr = f.replace("tcm-", "").replace(".html", "");
      let display = dateStr;
      let weekday = "";
      try {
...
<title>TCM Psychiatry · 中醫精神醫學文獻日報</title>
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .links-banner { margin-top: 48px; display: flex; flex-direction: column; gap: 12px; }
  .banner-link { display: flex; align-items: center; gap: 14px; padding: 14px 20px; background: var(--surface); border: 1px solid var(--line); border-radius: 12px; text-decoration: none; color: var(--text); transition: all 0.2s; font-size: 15px; }
  ...
  .banner-name { font-weight: 600; flex: 1; }
  .banner-arrow { color: var(--accent); font-weight: 700; }
...
<div class="container">
  <div class="logo">🌿</div>
  <h1>TCM Psychiatry</h1>
  <p class="subtitle">中醫精神醫學文獻日報 · 每日自動更新</p>
  ...
  <footer>
    <p>Powered by PubMed + NVIDIA AI · <a href="https://github.com/u8901006/tcm-psychiatry">GitHub</a></p>
  </footer>
</div>
</body>
</html>`;

  writeFileSync(`${docsDir}/index.html`, html);
